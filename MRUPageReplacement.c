...
int mruSearch(pageTable PT[],int n)
{
    int frame_ind = -1 , ind=-1 , max_lru = INT_MIN  , i ;
    for(i=0;i<n;i++)
    {
        if(PT[i].valid == 1 && PT[i].accessTime > max_lru){
            ind = i ;
            frame_ind = PT[i].frame ;
            max_lru = PT[i].accessTime ;
        }
    }
...
int main()
{
    ...
    for(i=0;i<n;i++)
    {
        ...
        if(ind==-1)
        {
            ...
            {
                int mru_ind = mruSearch(PT,size);
                frame[mru_ind] = page[i] ;
                PT[page[i]].accessTime = i ;
                PT[page[i]].valid = 1 ;
                PT[page[i]].frame = mru_ind ;
                current++;
            } 
        }
        else{
